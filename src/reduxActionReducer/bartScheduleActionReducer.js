// action creator;
export function getBartStationId (stationAddress){
  let station = stations.find(function(station){
    let address = `${station.address}`.replace(/\s/g, '').toLowerCase();
    stationAddress = stationAddress.replace(/\s/g, '').toLowerCase();
    return stationAddress.indexOf(address.substr(0, 10)) > 0;
  });
  if (station){
    return station.abbr;    
  }
...
export const getBartSchedule = (bartStationId) => {    
    return (dispatch) => {
      let url = '';
      if (bartStationId) {
        url = `https://api.bart.gov/api/etd.aspx?cmd=etd&orig=${bartStationId}&key=MW9S-E7SL-26DU-VV8V&json=y`
        fetch(url)
        .then(response => {
          return response.json();
        }).then(bartStationSchedule=>{
          dispatch(getBartScheduleSuccess(bartStationId, bartStationSchedule));
          return ('')
        });
      }
  }
...
export function getBartScheduleSuccess(bartStationId, bartStationSchedule) {
    let newState = {}      
    newState[bartStationId] = bartStationSchedule;
    return {
        type: 'GET_BART_SCHEDULE_SUCCESS',
        state: newState
    };
}

export function resolveBartScheduleResponseFromProp(props, bartStationId){
  return props.bartSchedule[bartStationId] || null;
}

// reducer to return a state
export default (state = {}, action) => {
    switch (action.type){
      case 'GET_BART_SCHEDULE_SUCCESS':
